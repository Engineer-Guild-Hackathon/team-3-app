...
const log = getLogger('api:v1:tag-mastery');
...
export async function GET(request: Request) {
  ...
  try {
    const items = await db
      ...
      .from(userTagMastery)
      .where(eq(userTagMastery.userId, result.user.userId))
      .orderBy(asc(userTagMastery.tagId));

    return jsonResponse({ items }, { cors });
  } catch (error) {
    log.error({ msg: 'tag_mastery:list_failed', userId: result.user.userId, err: error instanceof Error ? error.message : String(error) });
    return errorResponse('internal_error', 'Failed to load tag mastery.', { status: 500, cors });
  }
}
