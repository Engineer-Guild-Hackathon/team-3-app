...
const fieldsBySubject: Record<string, Field[]> = {
  math: [
    { id: 'algebra', name: '代数' },
    { id: 'geometry', name: '幾何' },
    { id: 'calculus', name: '微積分' },
    { id: 'statistics', name: '統計' },
  ],
  english: [
    { id: 'grammar', name: '文法' },
    { id: 'reading', name: '読解' },
...
export function FieldMultiSelect({ selectedFields, onFieldsChange, subject, disabled = false }: FieldMultiSelectProps) {
  const availableFields = fieldsBySubject[subject] || [];
                ...
                >
                  ...
                  {!disabled && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.9 }}
                      className="w-4 h-4 rounded-full bg-blue-500/20 hover:bg-blue-500/30 flex items-center justify-center"
                    >
                      <X className="w-3 h-3" />
                    </motion.button>
                  )}
      ...
      `}>
        {!subject ? (
          ...
          <div className="space-y-2">
            <label className={`block ${disabled ? 'text-gray-400' : 'text-gray-700'}`}>
              分野を選択（複数選択可）
            </label>
